...
Например, для бондинга беспроводной сетевой карты и обычной в режиме failover, т.е. при отсутствии интернета по кабелю используется wifi, при возврате кабеля - используется он. ``` 
`/etc/network/interfaces`:
```
auto eth0
...
```
# 5
Маска /29 определяет сеть с 8 ip адресами: 1 определяет сеть, 1 для бродкаста, 6 для хостов  
Можно уложить 32 подсети /29  в сети /24  
Например, 10.10.10.0/29, 10.10.10.8/29, 10.10.10.16/29,...
# 6
Например, `100.64.0.0/26`
# 7
Windows `arp -a`
Linux `arp`, очистить полностью – `sudo ip -s -s neigh flush all`, удалить один – `sudo arp -d айпихоста`
